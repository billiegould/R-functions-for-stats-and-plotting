...
### enhanced lower triangle of a 'pairs' multiple correlation plot {base}
## BG 2023
##' @param df a data frame with data columns to correlate
##' @param col data columns to plot
##' @param cex.labels magnification for text labels
##' @param cex.points magnification for plot points
##' @param cex.corr magnification for correlation value labels
##' @param legend whether to add a legend, default: TRUE
##' @param legend.x x.coord of upper right corner of legend: default 0.7
##' @param legend.y y.coord of upper right corner of legend: default 0.8
##' @param color.by required data column to use for coloring points
##' @param colors optional named list of point colors by category, otherwise
##' randomly chosen colors
##' @param plot.title optional plot title
##' 
##' @return colors color set used for plotting
pairs_ <- function(df=NULL, cols=NULL, color.by=NULL, cex.labels=1.5, 
                   cex.points=2.5, cex.corr=2.0,
                   legend=TRUE, legend.x=0.7, legend.y=0.8, 
                   colors=NULL, plot.title=""){
  
  if (is.null(colors)){
    if (!(is.null(color.by)) & color.by %in% names(df)){
      warn_na(df[[color.by]])
      colors = get_random_color_dict(names = unique(df[[color.by]]))
    }else{
      warning(glue("Did not specify a color.by variable: {color.by}"))
      colors=rep(nrow(df), "black")
      names(colors) <- row.names(df)
      df = df %>% mutate(rownames=row.names(df))
      color.by = "rownames"
    }
  }
  warn_na(df %>% select(all_of(cols)))
  lower.panel<-function(x, y){
    points(x, y, pch=19, cex = cex.points, col = colors[df[[color.by]]])
    r <- round(cor(x[!is.na(y)&!is.na(x)], y[!is.na(x)&!is.na(y)]), digits=2)
    #print(r)
    txt <- paste0("R = ", r)
    par(usr = c(0, 1, 0, 1)) 
    par(cex.axis=cex.labels)
    text(0.5, 0.9, txt, cex=cex.corr)
  }  
  pairs(df %>% select(all_of(cols)) %>% select(-{{color.by}}),
    cex.labels = cex.labels,
    upper.panel=NULL, lower.panel=lower.panel,
    main=plot.title)
  if (legend){
    legend(legend.x, legend.y, as.vector(unique(df[[color_by]])), 
           fill=colors, title = color.by)
  }
  return(colors) ## TODO: not sure how to get a plot object out of this base function
}
